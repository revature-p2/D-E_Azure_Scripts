...
username=$3 # name of resource group
...
result4=$(az sql server list --query [].name | grep -E /EricDavidsqlserver/) # since EricDavidsqlserver is static
if [ -n "$result4" ]; then
    echo "EricDavidsqlserver exists"
    exit 1
fi
# name of server will be all lowercase
az sql server create --location southcentralus -g $username --name EricDavidsqlserver --admin-user $username --admin-password $password
